...
from core.utils import assign_to_cig  # Ensure this function is defined per your business logic

User = get_user_model()

class Command(BaseCommand):
    help = ("Generate dummy data for Machakos County with 40 wards, 5 localities per ward, "
            "an Agro-Technician per locality, and dummy Farmer/Farm/FarmProduce data.")
        ...
        for ward_num in range(1, 41):  # Wards 1 to 40
            ...
            for loc_num in range(1, 6):  # 5 localities per ward
                ...
                if not User.objects.filter(username=agro_username).exists():
                    ...
                    self.stdout.write(f"    Agro-Technician already exists: {agro_username}")
                ...
                # Create dummy farmers and their farms for this locality.
                for farm_num in range(1, num_farms + 1):
                    ...
                    if not User.objects.filter(username=farmer_username).exists():
                        ...
                        farm = Farm.objects.create(
                            farmer=farmer_obj,
                            name=f"Farm {ward_num}-{loc_num}-{farm_num}",
                            locality=locality
                        )
